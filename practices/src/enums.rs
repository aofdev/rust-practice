...
fn process_payment(some_payment: Payment) {
  match some_payment {
    ...
    Payment::DebitCard(data) => {
      println!("Paying with debit card_number {}, amount of {}", data.card_number, data.amount);
    }
    Payment::Crypto{account_id, amount} => {
      println!("Paying with crypto account_id {}, amount of {}", account_id, amount);
    }
  }
}
...
pub fn run() {
  ...
  let crypto_payment = Payment::Crypto{account_id : "aof".to_string(), amount : 100.};
